...
dim bol as boolean
dim sVariable, sValue, sCondition as String
...
for iLine = StartLine to CountFields(Script,chr(13))
	...
	//Nur wenn RunCmd dann erstes und letztes Zeichen loeschen
	...
	if Device = "" then
		...
		if Command = "WaitForVariable" then
			WaitForVariable(NthField(DP1," ",1),NthField(DP1," ",2),NthField(DP1," ",3))
		end
	end
Next
